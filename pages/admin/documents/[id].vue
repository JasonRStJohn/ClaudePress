<template>
  <div class="max-w-xl">
    ...
    <form v-if="doc" class="space-y-5" @submit.prevent="handleSubmit">
      ...
      <div>
        <div class="block text-sm font-medium text-slate-700 mb-1">Current file</div>
        <div v-if="currentFileName" class="text-sm text-slate-600 bg-slate-50 border border-slate-200 rounded-md px-3 py-2 mb-2">
          {{ currentFileName }}
        </div>
        <label class="block text-sm font-medium text-slate-700 mb-1" for="file">Replace file (optional)</label>
        <input
          id="file"
          ref="fileInput"
          type="file"
...
const doc = ref<RecordModel | null>(null)
...
onMounted(async () => {
  const [record, cats] = await Promise.all([
    pb.collection('documents').getOne(id).catch((e: any) => {
      loadError.value = e?.message || 'Document not found.'
      return null
    }),
